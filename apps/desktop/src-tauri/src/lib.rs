...
fn get_about_info_inner(config_dir: Option<String>) -> Result<AboutInfo> {
    ...
    let native_updater_supported = std::env::current_exe()
        .ok()
        .and_then(|path| path.parent().map(|parent| parent.join("Astra.portable")))
    ...
    Ok(AboutInfo {
        ...
        codex_dir: codex_dir.display().to_string(),
        project_url: "https://github.com/alexbieber/AstraX".to_string(),
        github_repo: "alexbieber/AstraX".to_string(),
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .expect("error while building Astra");
