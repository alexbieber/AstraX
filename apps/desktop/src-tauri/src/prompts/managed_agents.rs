...
pub(crate) fn managed_agents_bounds(content: &str) -> Result<Option<(usize, usize)>> {
    const LEGACY_BEGIN: &str = "<!-- CODEX-X:INSTRUCTIONS:BEGIN -->";
    const LEGACY_END: &str = "<!-- CODEX-X:INSTRUCTIONS:END -->";

    let begin_markers = [AGENTS_MANAGED_BEGIN, LEGACY_BEGIN];
    let end_markers = [AGENTS_MANAGED_END, LEGACY_END];

    let begins = begin_markers
        .iter()
        .flat_map(|marker| content.match_indices(marker).map(|(index, _)| index))
        .collect::<Vec<_>>();
    let ends = end_markers
        .iter()
        .flat_map(|marker| {
            content
...
pub(crate) fn managed_agents_template_key_from_content(content: &str) -> Option<String> {
    const LEGACY_TEMPLATE_PREFIX: &str = "<!-- CODEX-X:TEMPLATE:";
    ...
    content[start..end].lines().find_map(|line| {
        ...
        trimmed
            ...
            .or_else(|| trimmed.strip_prefix(LEGACY_TEMPLATE_PREFIX))
